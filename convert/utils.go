...
func Patch(b []byte, s []singbox.SingBoxOut, urltestOut bool, include, exclude string, extOut []interface{}, extags ...string) ([]byte, error) {
	d, err := PatchMap(b, s, include, exclude, extOut, extags, urltestOut, true)
	if err != nil {
...
func PatchMap(
	...
	confTemplate, err := parseTemplate(tpl)
	if err != nil {
		return nil, fmt.Errorf("PatchMap parseTemplate faild: %w", err)
	}

	fileredOutbounds, emptyOutbounds, err := filerOutbounds(confTemplate, ftags)
	if err != nil {
...
func parseTemplate(tpl []byte) (singbox.TemplatePartialConfig, error) {
	var partialConfig singbox.TemplatePartialConfig
	err := json.Unmarshal(tpl, &partialConfig)
	if err != nil {
		return singbox.TemplatePartialConfig{}, fmt.Errorf("parseTemplate: %w", err)
	}
	return partialConfig, nil
}

func filerOutbounds(partialConfig singbox.TemplatePartialConfig, ftags []string) ([]any, []string, error) {
	...
	if len(partialConfig.Outbounds) > 0 {
		...
		custemNodesTypes := []string{"vmess", "shadowsocks", "trojan", "snell", "socks5", "http", "https", "ssr", "vless", "wireguard"}
		for _, outbound := range outboundsTemp {
			if contains(custemNodesTypes, outbound.Type) {
				ftags = append(ftags, outbound.Tag)
			}
		}
		...
		for _, outbound := range outboundsTemp {
			if outbound.Filter != nil {
				for _, filterInfo := range *outbound.Filter {
					...
					if len(filtered) > 0 {
						outbound.Outbounds = append(outbound.Outbounds, filtered...)
						outbound.Filter = nil
